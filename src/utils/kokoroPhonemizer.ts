...
interface PhonemizerModuleLike {
...
let phonemizerModule: PhonemizerModuleLike | null = null;
let phonemizerLoadError: Error | null = null;
let nativePhonemizerModule: NativePhonemizerModuleLike | null = null;
let nativePhonemizerLoadError: Error | null = null;

function getPhonemizerExport(moduleExports: unknown): PhonemizerModuleLike['phonemize'] | null {
  if (moduleExports && typeof moduleExports === 'object') {
    ...
    if (typeof moduleRecord.phonemize === 'function') {
      return moduleRecord.phonemize as PhonemizerModuleLike['phonemize'];
    }

    if (typeof moduleRecord.default?.phonemize === 'function') {
      return moduleRecord.default.phonemize as PhonemizerModuleLike['phonemize'];
    }
  }
...
function loadPhonemizerModule(): PhonemizerModuleLike {
  if (phonemizerModule) {
    return phonemizerModule;
  }

  if (phonemizerLoadError) {
    throw phonemizerLoadError;
  }

  try {
    // Keep the dependency cold until Kokoro is actually used.
    const moduleExports = optionalRequire<unknown>(() => require('phonemizer'));

    if (!moduleExports) {
      throw new Error('Missing phonemizer module.');
    }

    const phonemize = getPhonemizerExport(moduleExports);

    if (!phonemize) {
      throw new Error('Missing phonemize export.');
    }

    phonemizerModule = { phonemize };
    return phonemizerModule;
  } catch (error) {
    phonemizerLoadError = normalizePhonemizerLoadError(error);
    throw phonemizerLoadError;
  }
}
...
export async function phonemizeForKokoro(
  ...
  const { phonemize } = loadPhonemizerModule();
